    ...
    while ($fi = mysqli_fetch_array($resul)) {
      // Añadir cada fila al array con una estructura correcta
        $entry = [
          ...
          "pa"=>$fi["pa"],
          "fr"=>$fi["fr"],
          "motivo_consulta"=>$fi["motivo_consulta"],
...
if(isset($_SESSION["tipo_usuario"]) && $_SESSION["tipo_usuario"]=='admision')
{
  ...
  if(isset($_GET["accion"]) && $_GET["accion"]=="rnnh"){
    $hc->RegistrarNuevoHistorial($_POST["paciente_rd"],$_POST["cod_rd"],$_POST["nombre_historial"],$_POST["cod_his"],$_POST["subnombre"],$_POST["tipoHistorial"]);
  }
